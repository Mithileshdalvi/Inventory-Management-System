...
def myprofile(option):
    if 'username' in session:
        ...
        if option == 2:
            cur.execute(
                "SELECT `Emailid` FROM `user` WHERE `username` = '" + userid + "'")
        if option == 3:
            cur.execute(
                    ...
                    if not any(char.islower() for char in password):
                        ...
                        if len(contact_no) < 10:
                            ...
                            if password != pass_word:
                                ...
                                if password == pass_word:
                                    cur.execute("INSERT INTO `user`(`username`,`password`,`name`,`Emailid`,`contact_no`) VALUES (%s,"
...
def ForgotPassword():
    if request.method == "POST":
        ...
        print(strUid)
        #
        cur.execute("SELECT `Emailid` FROM `user` WHERE `username` = '" +
                    username + "'")
...
def dashboard():
    if 'username' in session:
        ...
        total_Invoicespending = CountDetailsOfInventory(4)
        total_earning = CountDetailsOfInventory(5)
        total_Lessproduct = CountDetailsOfInventory(6)

        # checking if stocks are empty or close to minlimit
        checkStocks()

        return render_template("dashboard.html", username=username, date=displayDate, total_product=total_product, total_suppliers=total_suppliers, total_invoices= total_invoices, total_spending= total_Invoicespending, total_profit= total_earning, product_name=total_Lessproduct)
    else:
...
def GetBilldetail():
    if 'username' in session:
        ...
        return render_template("billing.html", username=username, date=displayDate, bill_detail=BillProducts, total_amount=totalBill, bill_items=BillProducts)
    else:
        return redirect("login")
...
def calcChange():
    ...
    return render_template("billing.html", returnMoney=returnMoney)
